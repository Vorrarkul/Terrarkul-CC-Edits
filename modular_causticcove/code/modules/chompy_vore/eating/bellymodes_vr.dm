...
/////////////////////////// Exit Early ////////////////////////////
	...
	for(var/mob/dead/observer/G in touchable_atoms) // don't bother trying to process ghosts.
		touchable_atoms -= G
		...
		if((LAZYLEN(EL) || LAZYLEN(emote_lists[DM_HOLD_ABSORBED]) || (digest_mode == DM_DIGEST && LAZYLEN(emote_lists[DM_HOLD])) || (digest_mode == DM_SELECT && (LAZYLEN(emote_lists[DM_HOLD])||LAZYLEN(emote_lists[DM_DIGEST])||LAZYLEN(emote_lists[DM_ABSORB])) )) && next_emote <= world.time)
			...
			for(var/mob/living/M in contents)
				if(M.absorbed)
					...
					if(LAZYLEN(EL))
						to_chat(M, span_notice(belly_format_string(EL, M, use_absorbed_count = TRUE)))
				else
					...
					if(LAZYLEN(EL))
						to_chat(M, span_notice(belly_format_string(EL, M)))
...
/obj/belly/proc/prey_loop()
	for(var/mob/living/M in belly_surrounding) //contents changed to belly_surrounding to loop sound for indirect viewers too
		...
		if(is_wet && wet_loop && (world.time > M.next_preyloop)) //Removed isbelly(M.loc) as some viewers might be indirectly in the belly
			...
			var/sound/preyloop = sound('modular_causticcove/sound/cvore/vore/sunesound/prey/loop.ogg')
	...
	if(!instant && slow_digestion) // Gradual corpse digestion
		if(!M.digestion_in_progress)
			...
			if(M.health > -36 || (ishuman(M) && M.health > -136))
				to_chat(M, span_notice("(Your predator has enabled gradual body digestion. Stick around for a second round of churning to reach the true finisher.)"))
		...
		if(M.digestion_in_progress)
			return
	var/digest_alert_owner = span_notice(belly_format_string(digest_messages_owner, M))
	var/digest_alert_prey = span_notice(belly_format_string(digest_messages_prey, M))
