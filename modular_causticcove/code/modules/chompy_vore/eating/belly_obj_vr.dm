...
/obj/belly
	...
	var/max_reagents = 500							//How much metabolism content for full overlay.
	var/reagents_color = "#664330"					//Normal color holder for reagents layer. Blended from existing reagent colors.
	var/custom_reagents_color = null				//Custom color for reagents reagent layer.
	var/custom_reagents_alpha = 255					//Custom alpha for reagents reagent layer if not using normal mush layer.
...
/obj/belly/vars_to_save()
	...
	"max_reagents",
	"custom_reagents_color",
	"custom_reagents_alpha",
	"gen_cost",
	"gen_amount",
	"gen_time",
	"gen_time_display",
...
/obj/belly/Entered(atom/movable/thing, atom/OldLoc)
	...
	if(owner && !cycle_sloshed && reagents.total_volume > 0)
	...
	for(var/mob/living/M in startfx) // End of indirect vorefx changes
		...
		if(raw_desc)
			...
			to_chat(M, span_notice(span_bold("[belly_format_string(raw_desc, M)]")))
		...
		if(reagents.total_volume >= 5)
			...
			to_chat(M, span_warning(span_bold("You splash into a pool of [reagent_name]!")))
...
/obj/belly/proc/release_specific_contents(atom/movable/M, silent = FALSE)
	...
	for(var/obj/item/micro/H in M.contents)
...
/obj/belly/proc/nom_mob(mob/prey, mob/user)
	...
	prey.forceMove(src)
	/*if(ismob(prey))TODO figure out what is going on
		var/mob/ourmob = prey
